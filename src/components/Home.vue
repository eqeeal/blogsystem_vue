<template>
  <el-container>
    ...
    <el-aside :class="iSCollapse ? 'isCollapse' : 'notCollapse'">
      ...
      <!-- 博主名 -->
      <div class="head-left">
        <img :src="user.userAvatar" class="avatar" />&nbsp&nbsp
        <span v-show="!iSCollapse">{{user.userName}}</span>
      </div>
      <!-- 菜单 -->
      <el-menu
    ...
    <el-container>
      ...
      <el-header>
        ...
        <div class="head-right">
          <i class="el-icon-s-promotion dashbord" style="font-size: 25px"></i>
          <div @click="goToDoor()">博客首页</div>
          <i class="el-icon-user-solid dashbord" style="font-size: 25px"></i
          >作者
        </div>
      </el-header>
      <!-- 右侧内容区域 -->
...
export default {
  ...
  methods: {
    ...
    getUserInfo(){
      this.$http.get("/user/getInfoById?id="+localStorage.getItem("userId")).then(res=>{
